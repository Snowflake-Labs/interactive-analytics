...
```
clusters_min_count = ceil(concurrent_queries / MAX_CONCURRENCY_LEVEL)
clusters_max_count = clusters_min_count * 2
```

**Example:** 50 concurrent queries → `ceil(50 / 8) = 7 clusters` → `MIN_CLUSTER_COUNT = 7`, `MAX_CLUSTER_COUNT = 14`.

Set with `MIN_CLUSTER_COUNT = 7` and `MAX_CLUSTER_COUNT = 14` (min clusters × 2) and `SCALING_POLICY = STANDARD`. MCW will spin extra clusters up on demand during the burst and back down when idle.

## Levers that change the answer

| Lever | Effect | Example |
|-------|--------|---------|
| **MAX_CONCURRENCY_LEVEL** | For tiny queries (sub-second, small scan), you can safely raise it (e.g. 16). | With MCL=16: `ceil(50/16) = 4` → MIN=4, MAX=8. Watch for CPU contention — if per-query XP time inflates, MCL is too high. |
